...
export default async function MessagesPage() {
  ...
  const threadsWithLatestMessage = (rentals ?? [])
    .map((rental) => {
      const messages = (rental as unknown as { messages: { id: string; content: string; created_at: string; is_read: boolean; sender_id: string }[] }).messages ?? []
    ...
    .sort((a, b) =>
      b.latestMessage!.created_at.localeCompare(a.latestMessage!.created_at)
    )
      ...
      {threadsWithLatestMessage.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {threadsWithLatestMessage.map((thread) => {
            const renter = (thread as unknown as { renter: { id: string; name: string | null; avatar_url: string | null } }).renter
            const owner = (thread as unknown as { owner: { id: string; name: string | null; avatar_url: string | null } }).owner
            const costume = (thread as unknown as { costumes: { id: string; title: string; images: string[] } }).costumes
            ...
            return (
              ...
              >
                ...
                {thread.unreadCount > 0 && (
                  <span className="shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-bold text-white">
                    {thread.unreadCount}
                  </span>
                )}
              </Link>
